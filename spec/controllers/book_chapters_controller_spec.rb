...
RSpec.describe BookChaptersController, type: :controller do
  let(:valid_attributes) do
    { 'author_first_name' => %w[Test Person], 'author_last_name' => %w[Case 2], 'college_ids' => ['', '1', '4'], 'uc_department' => 'Test', 'work_title' => 'Test', 'other_title' => 'Test', 'publisher' => 'Test', 'page_numbers' => 'Test', 'city' => 'Test', 'publication_date' => 'Test', 'url' => 'Test', 'doi' => 'Test', 'submitter_id' => submitter.id }
  end

  let(:invalid_attributes) do
    { 'author_first_name' => ['Bad'], 'author_last_name' => [''], 'college_ids' => [''], 'uc_department' => '', 'work_title' => '', 'other_title' => '', 'publisher' => '', 'page_numbers' => '', 'city' => '', 'publication_date' => '', 'url' => '', 'doi' => '', 'submitter_id' => submitter.id }
  end
  ...
  describe 'PUT #update' do
    ...
    context 'with valid params' do
      ...
      it 'updates the requested book chapter' do
        ...
        book_chapter.reload
        expect(book_chapter.author_first_name).to eql %w[Test Person] # verify unchanged
        expect(book_chapter.url).to eql 'www.cool.com'
        expect(book_chapter.college_ids).to eql [6, 7]
      end

      it 'redirects to the book_chapter' do
        put :update, params: { id: book_chapter.id, book_chapter: valid_attributes }
  ...
  describe 'DELETE #destroy' do
    ...
    it 'destroys the requested book_chapter' do
      expect do
        delete :destroy, params: { id: book_chapter.id }, session: valid_session
      end.to change(BookChapter, :count).by(-1)
    end
