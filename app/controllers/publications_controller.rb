...
class PublicationsController < ApplicationController
  ...
  def show
    ...
    publication_id = instance_variable_get("@#{controller_name.singularize}").id
    submitter_id = model_class.find(publication_id).submitter_id

    @submitter = helpers.find_submitter(submitter_id) if session[:admin]
  end
